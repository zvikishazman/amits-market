...
export default function QuestionDetailPage() {
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => {
      ...
      if (days > 0) setCountdown(`${days}${t("daysShort")} ${hours}${t("hoursShort")} ${minutes}${t("minutesShort")}`);
      else if (hours > 0) setCountdown(`${hours}${t("hoursShort")} ${minutes}${t("minutesShort")} ${seconds}${t("secondsShort")}`);
      else setCountdown(`${minutes}${t("minutesShort")} ${seconds}${t("secondsShort")}`);
    };
    updateCountdown();
  ...
  function openEditModal() {
    ...
    setEditClosesAt(question.closesAt ? new Date(question.closesAt).toISOString().slice(0, 16) : "");
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      if (newAmt !== question?.betAmount) updates.betAmount = newAmt;
      const newClose = editClosesAt ? editClosesAt : null;
      const oldClose = question?.closesAt ? new Date(question.closesAt).toISOString().slice(0, 16) : null;
      ...
      setMessage({ type: "error", text: err instanceof Error ? err.message : t("failedToSave") });
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="glass p-4 sm:p-6">
        ...
        <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-800 flex flex-wrap items-center gap-4 sm:gap-6 text-sm">
          ...
          {question.closesAt && (
            <div>
              <span className="text-[10px] sm:text-xs text-gray-500">{isClosed ? t("closed") : t("closesIn")}</span>
              <div className={`font-mono font-semibold text-base sm:text-lg ${isClosed ? "text-red-400" : "text-amber-400"}`}>{countdown}</div>
            </div>
          )}
        </div>
      </div>
          ...
          {question.options.map((option, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Resolve button for creator (inline) */}
                {isOpen && isCreator && !showResolveModal && (
                  <button
